...
def estimate_severity_level(data: np.array) -> np.array:
    ...
    res[vvi > 8] = 1
    res[np.logical_and(6 < vvi, vvi <= 8)] = 2
    res[np.logical_and(4 < vvi, vvi <= 6)] = 3
    res[np.logical_and(2 < vvi, vvi <= 4)] = 4
    return res
...
def estimate_average(data: np.array, mask=None) -> np.array:
    ...
    masked = cv2.bitwise_and(out, out, mask=mask2)
...
def load_model(model: str):
    return Classifier(os.path.join(os.getcwd(), "model", model))



def extract_coordinates(filename: str)-> None:
    ...
    coordinates["latitude"] = [float(i) for i in tags["GPS GPSLatitude"].values]
    coordinates["latitude ref"] = tags["GPS GPSLatitudeRef"].values
    coordinates["longitude"] = [float(i) for i in tags["GPS GPSLongitude"].values]
    coordinates["longitude ref"] = tags["GPS GPSLongitudeRef"].values
    coordinates["altitude"] = float(tags["GPS GPSAltitude"].values[0])
